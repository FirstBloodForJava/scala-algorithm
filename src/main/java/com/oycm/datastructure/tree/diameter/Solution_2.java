...
public class Solution_2 {
    ...
    public int minimumDiameterAfterMerge(int[][] edges1, int[][] edges2) {
        ...
        优化思路：第三种方式的直径为以下三部分之和
            节点 a 在 tree1 到最远点的距离最小值(树的高度)
            节点 b 在 tree2 到最远点的距离最小值
            节点 a 和 节点 b 的距离 1
        结论：树中，节点 a 在 树中到最远点的距离最小值为树中最长路径除以 2 上取整，记为 L/2↑, 且节点一定在最长路径上
        证明：最远距离最小值为 L/2↑，反证法，假设存在一个点，存在最远距离 l < L/2↑
            假设点 v 在最长路径上, max(vx, vy) <= l <= L/2↑ - 1, 与 xy = L = vx + vy 相矛盾(vx + vy <= L - 2)
            假设点 v 不在最长路径上, 点 v 始终会和最长路径上某一点 v' 相交，存在 max(vv' + v'x, vv' + v'y) <= h < L/2↑
            vv' + v'x <= h, vv' + v'y <= h => v'x + v'y < 2h, h 最大为 L/2↑ - 1, v'x + v'y 最大为 L - 2 与 vx + vy = L 相矛盾
        节点一定在最长路径上，反证法，假设节点 v 不存在 x,y 所在路径上，存在一个最小高度 h = L/2↑,
            v -> x 与 v -> y 一定存在一个交叉点 v'
            v'x = vv' + v'x; v'y = vv' + v'y, vv' max(v'x, v'y) <= h
            由于 vv' 肯定大于 1, 所以 v'x < h - 1, v'y < h - 1 => v'x + v'y < 2h - 1 = 2 L/2↑ - 1, 这与 x,y 距离为 L 相矛盾
