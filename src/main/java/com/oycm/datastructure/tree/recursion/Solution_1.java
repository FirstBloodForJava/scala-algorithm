package com.oycm.datastructure.tree.recursion;
...
public class Solution_1 {
    ...
    public long dfs(int cur, int fa, List<Integer>[] g, long preSum, int[] cost) {
        preSum += cost[cur];
        if (g[cur].size() == 1) {
            return preSum;
        }
        ...
        for (int next : g[cur]) {
            if (next != fa) {
                long childSum = dfs(next, cur, g, preSum, cost);
                if (childSum > max) {
                    max = childSum;
                    cnt = 1;
