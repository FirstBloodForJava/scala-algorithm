...
class LRUCache {
    ...
    /*
    关键是如何快速找到最先插入的节点，把它删除。
    链表可以快速的删除和插入节点。
    单向链表能实现吗？能快速找到头节点和尾节点，但是无法快速知道哪些节点没有被使用。
    使用双向链表：
        在 put 时，先判断 key 是否存在，
            如果不存在，判断是否超出容量，容量超出，则删除双向链表的尾部节点；再将新节点作为链表的头节点。
            如果存在，先从链表中删除节点，再将节点作为链表的头节点。
     */
    // 双向链表
    ...
    private void pushFront(Node x) {
        /*
        将节点 x 插入到哨兵节点的后面
        向把 节点 x 的前后顺序调整正确，再调整 x 的 prev，以及 x 的 next 指向
        d ↔ a => d ↔ x ↔ a
         */
        // dummy ← x
        x.prev = dummy;
