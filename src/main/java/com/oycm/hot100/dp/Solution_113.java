...
public class Solution_113 {
    ...
    /**
     * 309. <a href="https://leetcode.cn/problems/best-time-to-buy-and-sell-stock-with-cooldown/description/">买卖股票的最佳时机含冷冻期</a>
     *
     * @param prices
     * @return
     */
    ...
    public int dfs(int i, int flag, int[] prices, int[][] memo) {
        ...
        if (flag == 1) {
            ...
            两者取最大值
             */
            return memo[i][flag] = Math.max(dfs(i - 1, flag, prices, memo), dfs(i - 2, 0, prices, memo) - prices[i]);
        } else {
            /*
            第 i 天未持有股票的最大利润：
