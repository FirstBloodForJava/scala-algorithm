...
public class Solution_74 {
    ...
    public int findKthLargest(int[] nums, int k) {
        ...
        如何记录 pivot 在 nums 中的下标？
            先在 nums 子数组中找到随机下标，把这个下标的值和子数组第一个下标交换。
        如何进行交换？
            [l+1, r] 区间的元素需要和 pivot 比较并交换位置
            初始 i = l+1, j = r
            while(i <= j && nums[i] < pivot) i++;
            while(i <= j && nums[j] > pivot) j--;
            上面循环结束，有以下两种情况：
                i > j，此时 nums[j] < pivot 恒成立，为什么呢
                    如果第一个循环结束 i <= j 成立，则 nums[i] > pivot，i 已经不变了，此时 j = i-1，根据前面的循环 nums[i] < pivot 才会变大，所以 nums[j] < pivot
                    如果第一个循环结束 i <= j 不成立，则 i = j+1，根据前面的循环 nums[i] < pivot 才会变大，所以 nums[j] < pivot
            当 i <= j 时，此时 nums[i] >= pivot 且 nums[j] <= pivot 把 i,j 互相交换位置，同时 i++, j-- 双指针互相靠拢
            当 i > j 时，退出循环 如果前面 nums[i] > pivot, nums[j] < pivot 交换前 [l+1, i) 都是小于等于 pivot 的，交换后，++i > --j，说明 j 在 [l+1, i) 肯定是小于等于 pivot
         */
        int n = nums.length;
        // 目标下标
        int targetIndex = n - k;
    ...
    private int partition(int[] nums, int left, int right) {
        ...
        while (true) {
            ...
            // 此时 nums[i] >= pivot 或 i > j
            while (i <= j && nums[j] > pivot) {
                j--;
            }
            // 此时 nums[j] <= pivot 或 i > j
            if (i >= j) {
                break;
            }
